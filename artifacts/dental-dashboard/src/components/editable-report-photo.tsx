...
export function EditableReportPhoto({
  ...
  return (
    <>
      <button
        type="button"
        aria-label={`Edit photo for ${report.name}`}
        onClick={() => setOpen(true)}
        className={`group relative shrink-0 rounded-full overflow-hidden border bg-primary/10 hover:ring-2 hover:ring-primary/40 transition h-24 w-24 ${className}`}
      >
        {currentUrl ? (
          <img
            src={currentUrl}
