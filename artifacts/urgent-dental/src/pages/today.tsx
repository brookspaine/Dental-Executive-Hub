...
function OverviewTab() {
  ...
  const reload = async () => {
    try {
      setData(await api<Overview>("/urgent-dental/overview"));
      setError(null);
    } catch (e) {
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {slots.map((row, idx) => (
          <Top3Row
            ...
            onSave={async (text) => {
              await api(`/urgent-dental/top3/${idx + 1}`, {
                method: "PUT",
...
function DirectReportsTab() {
  ...
  const load = async () => {
    try {
      setPeople(await api<DirectReport[]>("/urgent-dental/direct-reports"));
      setError(null);
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
    }
  ...
  const addPerson = async () => {
    ...
    if (!name?.trim()) return;
    await api("/urgent-dental/direct-reports", {
      method: "POST",
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      {visiblePeople.map((p) => (
        <SectionAccordion
          ...
          onRename={async (name) => {
            await api(`/urgent-dental/direct-reports/${p.id}`, {
              method: "PATCH",
          ...
          onDelete={async () => {
            ...
            await api(`/urgent-dental/direct-reports/${p.id}`, { method: "DELETE" });
            load();
          }}
        />
      ))}
...
function ProjectsTab() {
  ...
  const load = async () => {
    try {
      setProjects(await api<Project[]>("/urgent-dental/projects"));
      setError(null);
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
    }
  ...
  const addProject = async () => {
    ...
    if (!name?.trim()) return;
    await api("/urgent-dental/projects", {
      method: "POST",
      body: JSON.stringify({ name: name.trim() }),
    });
    load();
      ...
      {projects.map((p) => (
        <SectionAccordion
          ...
          headerRight={
            <button
              ...
              onClick={async (e) => {
                ...
                await api(`/urgent-dental/projects/${p.id}`, {
                  method: "PATCH",
          ...
          onDelete={async () => {
            ...
            await api(`/urgent-dental/projects/${p.id}`, { method: "DELETE" });
            load();
          }}
        />
      ))}
...
function LifeAreasTab() {
  ...
  const load = async () => {
    try {
      setAreas(await api<LifeArea[]>("/urgent-dental/life-areas"));
      setError(null);
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
    }
      ...
      {areas.map((a) => (
        <SectionAccordion
          ...
          bodyPrefix={
            <LifeAreaPanels
              area={a}
              apiPrefix="/urgent-dental"
              onAreaChange={load}
            />
          }
          onRename={async (name) => {
            await api(`/urgent-dental/life-areas/${a.id}`, {
              method: "PATCH",
              body: JSON.stringify({ name }),
            });
            load();
...
/* (kept in sync with the same component in the Dental CEO Dashboard).        */
/* ========================================================================== */
...
function LifeAreaPanels({
  ...
  const loadGoals = async () => {
    try {
      ...
      /* fail soft so the rest of the accordion still renders */
    }
  };
  useEffect(() => {
    loadGoals();
...
function SectionAccordion({
  ...
  /* Optional content rendered inside the expanded body above the task list.
     Used by Life Areas to surface Identity/Why/Goals panels. */
  ...
  const load = async () => {
    const [taskRows, sectionRows, drRows, projRows] = await Promise.all([
      api<Task[]>(`/urgent-dental/tasks?parentType=${parentType}&parentId=${parentId}`),
      api<TaskSection[]>(
        `/urgent-dental/task-sections?parentType=${parentType}&parentId=${parentId}`,
      ),
      parentType === "project"
        ? api<DirectReport[]>("/urgent-dental/direct-reports")
        : Promise.resolve([] as DirectReport[]),
      parentType === "direct_report"
        ? api<Project[]>("/urgent-dental/projects")
  ...
  const addSection = async () => {
    ...
    await api("/urgent-dental/task-sections", {
      method: "POST",
...
function TaskSectionGroup({
  ...
  const toggleCollapsed = async () => {
    if (!section) return;
    await api(`/urgent-dental/task-sections/${section.id}`, {
      method: "PATCH",
  ...
  const deleteSection = async () => {
    ...
    if (
      ...
      return;
    await api(`/urgent-dental/task-sections/${section.id}`, { method: "DELETE" });
    onChange();
  };
          ...
          {tasks.map((task) => (
            <TaskRow
              ...
              onUpdate={async (patch) => {
                await api(`/urgent-dental/tasks/${task.id}`, {
                  method: "PATCH",
                  body: JSON.stringify(patch),
                });
                onChange();
              ...
              onDelete={async () => {
                await api(`/urgent-dental/tasks/${task.id}`, { method: "DELETE" });
                onChange();
              }}
            />
          ))}
...
function BrainDumpTab() {
  ...
  const load = async () => {
    try {
      const [rows, drs, projs, inbox, ref, someday, processed] = await Promise.all([
        api<BrainDumpEntry[]>(`/urgent-dental/brain-dump?filter=${filter}`),
        api<DirectReport[]>("/urgent-dental/direct-reports"),
        api<Project[]>("/urgent-dental/projects"),
        api<BrainDumpEntry[]>("/urgent-dental/brain-dump?filter=inbox"),
        api<BrainDumpEntry[]>("/urgent-dental/brain-dump?filter=reference"),
        api<BrainDumpEntry[]>("/urgent-dental/brain-dump?filter=someday"),
        api<BrainDumpEntry[]>("/urgent-dental/brain-dump?filter=processed"),
      ]);
      setEntries(rows);
  ...
  const save = async () => {
    ...
    if (!text) return;
    await api("/urgent-dental/brain-dump", {
      method: "POST",
      body: JSON.stringify({ text }),
    });
    setDraft("");
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        {entries.map((e) => (
          <BrainDumpRow
            ...
            projects={projects}
            onUpdate={async (text) => {
              await api(`/urgent-dental/brain-dump/${e.id}`, {
                method: "PATCH",
                body: JSON.stringify({ text }),
              });
              load();
            ...
            onDelete={async () => {
              ...
              await api(`/urgent-dental/brain-dump/${e.id}`, { method: "DELETE" });
              load();
            }}
            onProcess={async (payload) => {
              await api(`/urgent-dental/brain-dump/${e.id}/process`, {
                method: "POST",
            ...
            onUnprocess={async () => {
              await api(`/urgent-dental/brain-dump/${e.id}/unprocess`, { method: "POST" });
              load();
            }}
          />
        ))}
