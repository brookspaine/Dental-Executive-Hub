...
- **Today** (`/ideal-week`): A Google Calendar-style weekly schedule supports drag-to-create, move, and resize time blocks, with half-hour grid lines and a current time indicator. Google Calendar integration overlays live events. Includes Today's Big 3 and This Week's Big 3 (each capped at 3 items), a **Brain Dump** backlog (uncapped, separate from the date- and week-scoped Big 3 lists; persists in `future_todos` table; completed items collapse behind a "Show N completed" toggle — adding an item here also fire-and-forget POSTs to `/api/command-center/brain-dump` so the entry is captured in Command Center's Brain Dump inbox), a Reading List, and editable Ritual sidebar sections. (The earlier Weekly Scorecard and Weekly Score progress bar were removed in June 2026.)
