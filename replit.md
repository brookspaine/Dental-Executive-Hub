...
- **EDGE Buildout Board**: Four-swimlane Kanban for tracking dental practice construction & launch tasks, organized by **Business Area** (Location/blue, Financials/green, People/violet, Operations/orange). Each swimlane is a self-contained 6-column board (Backlog → Ready → In Progress (WIP=5) → Waiting On → Review (WIP=3) → Done) with its own card count and per-swimlane WIP enforcement. Swimlane headers are sticky on vertical scroll and click-to-collapse (collapsed lanes still show per-column counts). Cards have universal fields (title, owner, category, business area, target date, definition of done, blocker, escalation trigger, KRA link) plus dynamic per-category fields stored in JSONB — only the original 6 categories (Lease & Legal, Permitting & DNR, Design & Construction, Signage, Equipment & IT, Vendor Contracts) have detail forms; new categories use universal fields only. Each business area exposes its own category dropdown (e.g. Financials → Capital & Funding, Banking & Treasury, Accounting & Tax, Insurance, Budget & Forecasting, Financial Reporting; People → Doctor Recruiting Partner Track, Doctor Recruiting Associate, Clinical Staff Hiring, Operations/Admin Hiring, Onboarding, Culture & Training, Performance Management, Compensation & Benefits). Drag-and-drop between columns within a swimlane changes status; dragging across swimlanes opens a Confirm/Cancel modal before changing the business area (and resets the category if the destination area doesn't accept the original category). Activity log appended atomically (Postgres `||` on JSONB) — auto-logs status changes, business-area changes, and edits, plus manual notes. "Weekly Review" view flags cards waiting >14 days, due in ≤7 days, past target, or with blockers, **grouped by Business Area** so leadership meetings can walk each area in sequence. Filter bar: search title/notes, owner, category, multi-select Business Area pills, blocked-only. Auto-seeds 10 spec cards on first load (transaction + advisory lock for safe concurrent startup).
...
- `buildout_cards` - EDGE Buildout Board cards with universal fields (title, owner_name, category, business_area NOT NULL, status, position, kra_link, target_done_date, definition_of_done, blocker, escalation_trigger), JSONB `category_fields` for category-specific data, JSONB `activity_log` for timestamped entries, `waiting_since` set when status enters 'waiting_on'. `business_area` is one of Location/Financials/People/Operations and determines which swimlane the card lives in.
