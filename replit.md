...
- **My Role**: Role index rendered as an org-chart tree (root tier at top, reports indented below with vertical connector lines). Each card shows the role title, seat holder (or "Open seat" with a dashed avatar), and Business Area / Tier pills. Roles mirror the Practice Org Chart hierarchy (two Owners, Office Manager, then Front Desk / Assistant / Lead Assistant / Associate Dentist / Practice Liaison). Role Detail pages include Purpose & Cultural Alignment, Key Results Area (a simple bullet list of measurable outcomes the seat is accountable for — mirrors the Practice Org Chart positions' KRA pattern), Daily Operations Protocol (checklists), Decisions to Own, and Playbooks & Procedures. Features a three-mode toggle: Reference (read-only), Daily (interactive checklists), and Edit (single-buffer form). New Role dialog includes a "Reports to" picker so additions slot into the tree.
